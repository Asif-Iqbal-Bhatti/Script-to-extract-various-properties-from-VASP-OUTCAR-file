...
USAGE='A simple script to generate FCC or BCC supercell for HEAs. \
To execute just run python3 <bcc/fcc> <c/d>. \
HEAs consists of five or more elements so it has already been typed into the \
script just change according to your problem also lattice vectors should be \
equal and atomic composition should corresponds to integer multiple of atoms.'
print (USAGE)
#------------------------------INPUT PARAMETRS------------------------------------#
lattice_parameter = 3.405 ; count=0
supercellx = 5
supercelly = 5
supercellz = 5
#----------------------------------------------------------------------------------#

cartesian_units=[]; reduced_units=[]; randStruct=[]
...
with open("newPOSCAR","w") as fdata:
	...
	for i in range(0,len(cartesian_units)):
